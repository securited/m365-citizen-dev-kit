...
| `deploy/` | Two deploy scripts, identical except for how the custom-script window opens: `Deploy-SampleLibrary.ps1` (tenant admin) and `Deploy-SampleLibrary.SelfService.ps1` (queues to the enablement Function App, no admin rights). `Test-Anonymization.ps1` is the pre-push anonymization gate; `examples/` holds the copy-me-for-a-new-app deploy script and the standalone enablement-window opener | No |
...
# Pick ONE — they differ only in how the custom-script window is opened.
deploy/Deploy-SampleLibrary.ps1 -WhatIf              # no enablement service: needs tenant admin
deploy/Deploy-SampleLibrary.SelfService.ps1 -WhatIf  # enablement Function App: needs no admin rights
```
...
Neither deploy script ships a built-in Entra app id — pass `-PnPClientId <guid>` or set `PNP_CLIENT_ID`. `.aspx` shell uploads need a custom-script enablement window (24h); `_data/` file updates do not — so both scripts compare local shells against the remote inventory and skip the window step entirely when none changed (`-ForceEnablement` overrides). The two scripts are deliberate siblings, not a base and a variant: everything after the enablement step is identical, so a change to the deploy engine must be made in **both**. See [deploy/README.md](deploy/README.md) for which to use, and [docs/script-enablement-self-service.md](docs/script-enablement-self-service.md) for the Function App's architecture and security model.
