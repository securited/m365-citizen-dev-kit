...
Neither deploy script ships a built-in Entra app id — pass `-PnPClientId <guid>` or set `PNP_CLIENT_ID`. `.aspx` shell uploads need a custom-script enablement window (24h, the script opens it); `_data/` file updates do not. The two scripts are deliberate siblings, not a base and a variant: everything after the enablement step is identical, so a change to the deploy engine must be made in **both**. See [deploy/README.md](deploy/README.md) for which to use, and [docs/script-enablement-self-service.md](docs/script-enablement-self-service.md) for the Function App's architecture and security model.
